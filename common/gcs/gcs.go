...
func (c *GCSClient) createClient(ctx context.Context) (*storage.Client, error) {
	...
	if err != nil {
		logrus.Errorf("Failed to create GCS client: %v", err)
		return nil, err
	}
...
func (g *GCSClient) UploadFile(ctx context.Context, fileName string, data []byte) (string, error) {
	...
	defer func(client *storage.Client) {
		if err := client.Close(); err != nil {
			logrus.Errorf("Failed to close GCS client: %v", err)
			return
		}
	}(client)
	...
	if err != nil {
		logrus.Errorf("Failed to copy data to GCS: %v", err)
		return "", err
	}

	if err := writer.Close(); err != nil {
		logrus.Errorf("Failed to close GCS writer: %v", err)
		return "", err
	}
	...
	if err != nil {
		logrus.Errorf("Failed to update GCS object content type: %v", err)
		return "", err
	}
